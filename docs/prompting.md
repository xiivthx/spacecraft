...
**Note:** Lens pass = five decision jobs that write `## Lens pass` or `Lens pass skipped:` in `decisions.md` - not expertise cosplay. See `.cursor/skills/sc-discuss/references/lens-pass.md` and sc-storm (Tier 3). Testability pass, Strategy pass, and RCRCRC pass are the same class of discuss decision jobs (`## Testability pass` / `## Strategy pass` / `## RCRCRC pass` or skips) - not QA personas. See `requirement-testability.md`, `htsm-strategy.md`, and `rcrcrc-impact.md`.
