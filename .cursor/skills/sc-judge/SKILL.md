...
- **Visual UI uncertain / live gap** - `uncertain` on draft-parity, **live-product**, or any required UX/UI dimension ⇒ `REFUTED` (fail-closed). Missing fresh live product evidence on visual ready ⇒ `REFUTED`. No third verdict; record `uncertain` in hunt reasons only.
...
- **Must**: Hunt for weakened tests, false completion, unauthorized action, mission-review dimensions per `.cursor/skills/sc-run/references/mission-review-gates.md`, and (when visual UI) draft drift per `.cursor/skills/sc-ux-design/references/ux-ui-review-gates.md` (use those phrases in findings so they are searchable).
- **Must**: On mission review, treat `uncertain` on any required dimension as `REFUTED` (fail-closed) - never `VERIFIED`.
- **Must**: On visual UI, treat `uncertain` draft-parity, **live-product**, or visual-ready claims as `REFUTED` (fail-closed) - never `VERIFIED`. REFUTE when fresh live product evidence is missing or **live-product** is fail/uncertain.
