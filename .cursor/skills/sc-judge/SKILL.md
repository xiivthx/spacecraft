...
2. **Collect claims (zero-context)** - Input is **structured-lines-only**: `spec.md` + design-contract + diff + evidence labels + extracted `decisions.md` lines only (grep prefixes: `Scenario oracle change:`, `Roadmap contract:`, `Cursor review:`, `Cursor review skipped:`, `Cursor ingest:`, `Sc-security fallback:`, `Loop watch:`, `Fact-check:`, debt/disposition lines such as `Mutation skipped:`, `Pbt skipped:`, `Characterization waived:`, `Debt ceiling:`). Treat each as a claim. **Never** free-text builder rationale, narrative task notes, or unprefixed `decisions.md` prose.
...
5. **Hunt (ready path)** - Search these targets only (do **not** re-walk the full mission-review dimension table; reviewer already applied `mission-review-gates` / UX gates):
   ...
   - **Loop watch disposition** - require greppable `Loop watch:` before ready (silence ⇒ `REFUTED`). When `Loop watch: ran` is present → require greppable `Loop watch: stopped:` **at judge time** (Commander Must disarm + write `stopped:` before invoking judge per `/sc-run`); `Loop watch: skipped:` alone after `ran` without any `stopped:` ⇒ `REFUTED`; coexisting `skipped:` does not REFUTE when `stopped:` is also present. When no `ran` → `Loop watch: skipped:` OK. Armed loop claimed after ready without disarm ⇒ `REFUTED`
   - **outcome-gate disposition** - greppable A/B/C + quality dispositions (`Design-contract:` / `Design-contract skipped:`, `Approved-scenarios:` / `Approved-scenarios skipped:`, `Static-analysis skipped:`, `Mutation skipped:`, `Pbt skipped:` / `Pbt waived:`, related evidence labels). Bars/skips: `docs/mission-artifacts.md`. Missing disposition without skip/waive ⇒ `REFUTED`. Inventing PBT lib mid-mission ⇒ `REFUTED`. Global 95-100% coverage as success bar ⇒ `REFUTED`. Tip-path-only lint/typecheck MUST NOT satisfy static-analysis for ready. REFUTE when tip-path-only static claimed sufficient
   - **HIL honesty** - REFUTE when aligned-only HIL claimed sufficient; REFUTE when single-DUT evidence under peer harness
   ...
   - **draft drift (visual UI only)** - when `UI draft approved:` recorded, REFUTE chrome divergence, missing paired draft+live screenshots, or fail/uncertain draft-parity / live-product. REFUTE when draft-parity: fail or uncertain for shared chrome. See `ux-ui-review-gates.md`; do not re-score every visual dimension here
...
7. **Cross-model critic disposition** - Best-effort second family (**not** a Must): `Cross-model critic: <family>` or `Cross-model critic skipped: no second family configured`. Never silent. Unresolved `Fact-check: contested:` used as an acceptance premise → `REFUTED`.
8. **Verdict**
   - `VERIFIED` - fresh evidence passes; scope matches; hunts clean; **0** review findings (any `source`); Cursor-review disposition present when required (and corroboration when disposition claims `ran`); greppable `Loop watch:` present (and `stopped:` when prior `ran`); dissent trail complete; cross-model disposition present; no unresolved `Fact-check: contested:` as acceptance. Ready allowed (subject to other gates).
   - `REFUTED` - any material hunt hit, failed re-run, scope/acceptance mismatch, leftover findings, missing Cursor-review disposition when required, missing ran-corroboration, incomplete dissent resolution, unresolved `Fact-check: contested:` as acceptance, or failed verify. Ready blocked.
...
- **Must**: Emit `VERIFIED` | `REFUTED` only; allow `ready` only on `VERIFIED`; prove from evidence + scope + hunts + empty findings + `validate --strict` - companion-lane success is never ready proof.
