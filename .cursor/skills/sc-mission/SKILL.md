...
4. **Route ambiguity** - If intent, scope, or acceptance criteria is ambiguous, route to `/sc-discuss` (sc-clarify protocol) before `/sc-run`. Apply mission sizing (`sc-discuss/references/mission-sizing.md`) before bind/run when scope may need phases or a feature-seam roadmap.
