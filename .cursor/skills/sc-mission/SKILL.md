...
1. **Resolve mission** - Run `spacecraft resolve [selector] [--json]`, `status`, or `missions`; `.space/current` is fallback state, not sole authority. Commander auto-detects development lane (Advisory, Mission, Debug, Quick) based on user intent.
...
- **Resolver safety ≠ safe** - Block all mutating work. Show conflicts. Require `spacecraft use <selector>`.
...
Set state with `spacecraft set-state <state>`. The CLI enforces valid transitions.

## Rules

- **Must**: Resolve the active mission with `spacecraft resolve [selector] [--json]`, `status`, or `missions`; `.space/current` is fallback state, not sole authority.
