...
4. **Route ambiguity** - If intent, scope, or acceptance criteria is ambiguous, route to `/sc-discuss` (sc-clarify protocol) before `/sc-run`.
5. **Enforce lifecycle** - Follow: mission -> `/sc-discuss` (clarify + spec + visual draft if needed) -> `/sc-run` (plan -> build -> verify -> review) -> ship. For roadmap AFK, `/sc-run` orchestrates jigsaw plan → per-acceptance RED-GREEN (checkpoint commits) → combine/refactor → review.
