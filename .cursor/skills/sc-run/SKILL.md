...
6. **UI recheck (visual UI/FE):** Start the app → Tier 3 on the **running product URL** (`playwright-cli` primary; open real product routes; screenshots at 375 / 768 / 1280, + 1536 when multi-region) → Step 0 **draft-parity** (side-by-side vs approved draft surface: tokens, layout, component chrome, scenario states) → Task(`sc-designer`) live critique (**live-product** + **draft-parity**) → fix critical/important → re-capture evidence → then review. Layout-only match with different chrome, or missing draft `data-state` coverage in the product, → fix now. Draft HTML serve alone does not satisfy live-product. Pair with the functional suite. No ready yet.
