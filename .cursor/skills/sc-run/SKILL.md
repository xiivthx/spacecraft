...
`spacecraft validate --strict`; matching `evidence.jsonl`; empty `review.json` findings; `sc-judge` `VERIFIED`; greppable `Cursor review:` or `Cursor review skipped:` (same grammar as step 9; when security in scope, skip valid only after `Sc-security fallback: pass` or SEC machine-evidence pass); UI/workflow in scope → `PROBE: CLEAN` (or probe skip). Canvas ≠ ready proof.
...
8. **browser-probe** (UI/workflow) - Task(`sc-browser-probe`) to `PROBE: CLEAN`; primary probe executor: Cursor IDE browser (`cursor-ide-browser`) via sc-browser-probe Browser matrix (then `playwright-cli`, then Chrome DevTools MCP); else handback. Skip when no runnable UI/workflow. Browser / MCP / chat success Must not authorize `ready` / `VERIFIED` / `AUTH` / ship (`PROBE:` is Spacecraft-owned; see sc-browser-probe).
9. **review** → **judge** - `validate --strict`; parallel Task(`bugbot`) + Task(`security-review`) before or as input to Task(`sc-reviewer`) (exact Cursor prompt: `Full Repository Path: <abs repo>`; `Diff: branch changes` default; optional `Base Branch` / `Custom Instructions` / bugbot-only `Change Description` per upstream skills - invoke + thin adapter only, do not copy skill bodies); write greppable `Cursor review: bugbot+security-review ran` or `Cursor review skipped: <reason>` (unavailable Cursor types ⇒ skip disposition). When writing `Cursor review: … ran`, Must also capture `spacecraft evidence "cursor-review-…" -- …` **or** write greppable `Cursor ingest: session` in `decisions.md`. When security is in scope, `Cursor review skipped:` is valid **only** after a greppable `Sc-security fallback: pass` (optionally also `Sc-security fallback: findings drained`; optional evidence label `sc-security-…`) **or** SEC machine-evidence pass; otherwise fail / ready blocked (skip alone does not satisfy security-when-in-scope). Map bugbot + security-review (+ sc-reviewer) outputs into `review.json` via `defect-finding.md` with `source` set; Task(`sc-reviewer`); sc-judge. Drain **all severities** (`critical`/`important`/`minor`): findings/`REFUTED` → fix (only when `requiredFix` is concrete; no free-form whole-tree refactor from Bugbot noise) → re-run Cursor reviews → re-review → re-judge until findings empty or `3-cycle`/`timebox` handback. Ready only on `VERIFIED` + empty findings: before `set-state ready`, if optional `sc-loop` watch was armed, Must disarm per `sc-loop` and emit `Loop watch: stopped: ready-handoff` (do not leave loop armed); if optional Automations+Slack via `sc-automate-slack` was armed, Must disarm per `sc-automate-slack` and emit `Automate-Slack: stopped: ready-handoff` (do not leave schedule/git Automation accepting Slack resume); then `set-state ready`. Handoff; never merge/push/tag.

Overnight: stop on `3-cycle` | `timebox` | `blocked` → `handback.md`. Optional overnight/AFK watch via `sc-loop` / Cursor `/loop` (CI/jobs); never required. On those stops: disarm loop (per `sc-loop`) + `handback.md`. Ready handoff must also disarm (do not leave loop armed). Optional `sc-loop` ticks never authorize `set-state ready` / ship; resume under step 9 gates only. Optional Automations+Slack HIL AFK via `sc-automate-slack` (Slack notify on handback/needs-HIL; Slack reply may cue resume under `/sc-run` gates); Slack resume ≠ AUTH; never required for ready/ship. On those stops or ready handoff: if Automations+Slack was armed, Must disarm per `sc-automate-slack` (do not leave schedule/git Automation accepting Slack resume). Optional canvas-SoT via `sc-canvas-sot` for plan | findings | evidence human-check emits; disposition `Canvas-sot: ran` | `Canvas-sot: skipped: <reason>`; never required for ready/ship or every `/sc-run`; Must not treat canvas as AUTH / `VERIFIED` / ready / ship authority; Canvas ≠ ready proof. Optional Goals-mirror via `sc-goal-roadmap` for multi-mission roadmaps (`Sizing: roadmap`); disposition `Goal-roadmap: ran` | `Goal-roadmap: skipped: <reason>`; never required for ready/ship or every roadmap discuss/run; Must not treat Goals as AUTH / `VERIFIED` / ready / ship authority; Goals ≠ ready proof. Checkpoint commits: one Conventional Commit per plan task after acceptances; see sc-git. Never push.

## References

- sc-tdd, sc-judge, sc-browser-probe, sc-loop (optional overnight `/loop` watch), sc-automate-slack (optional Automations+Slack HIL AFK; resume cue ≠ AUTH; never required), sc-canvas-sot (optional canvas-SoT plan|findings|evidence; `Canvas-sot:`; never required; Canvas ≠ ready proof), sc-goal-roadmap (optional Goals-mirror; `Goal-roadmap:`; never required; Goals ≠ ready proof), sc-split-to-prs (optional post-ready fat-diff split), `/sc-discuss`, `/sc-ship`, sc-git
