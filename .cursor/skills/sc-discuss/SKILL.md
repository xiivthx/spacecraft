...
6. **Polish winner** (sc-ux-design): full draft with scaffold (chrome outside framed surface); viewport toggles (375 / 768 / 1280 / 1536) with responsive CSS (not frame-resize-only at any preset); **surface-relevant scenario matrix** per `spec.md` + primary surface shape (happy path + failure/degraded the surface can enter; `loading` when async; `empty`/`few`/`many` when variable-length collection; plus feature/behavior surfaces from `spec.md`); applicable `- [ ]` items from the recorded id's file under `.cursor/skills/sc-ux-design/references/checklists/` as real chrome; real component chrome. Check all four viewports and **Responsive ladder** at each preset. Prompt assembly: `shared directives` → `DESIGN.md` → brief. Honor borrow scope - no silent full-chrome clone.
