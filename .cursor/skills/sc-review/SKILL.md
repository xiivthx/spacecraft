---
name: sc-review
description: "Reviews a mission diff and evidence when implementation is complete and release readiness must be assessed."
disable-model-invocation: true
---

Use sc-mission, sc-verification, sc-git, sc-solid, sc-performance, sc-security, and sc-architect.
...
If UI files changed, invoke /sc-designer as a read-only sidecar for design-risk triage. 
...
```
node .cursor/skills/sc-design/scripts/serve-html.mjs <artifact-or-dir> --open
```
...
Invoke /sc-reviewer as a read-only subagent.  The reviewer must not edit files.
...
- **Pass** → `spacecraft set-state ready`
- **Fail** (any critical finding) → `spacecraft set-state blocked`
