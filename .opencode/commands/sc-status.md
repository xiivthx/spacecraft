...
If resolver safety is not safe, do not recommend write/verify/review/ship work. Tell the user to choose with `scripts/spacecraft missions` then `scripts/spacecraft use <number|id|title>`, or use `SPACECRAFT_MISSION=<mission-id>` for one command.
