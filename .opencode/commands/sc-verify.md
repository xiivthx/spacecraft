...
1. Run focused verification using:
   ```
   scripts/spacecraft evidence "<label>" -- <command>
   ```
2. Capture evidence for tests, build, typecheck, or lint as appropriate. Capture failures too.
3. Update plan.json task evidence references when a check maps to a task.
4. Run:
   ```
