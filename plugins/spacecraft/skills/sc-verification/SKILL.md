...
- **Must** (mission product path before ready): ensure static-analysis, diff-coverage, mutation, and PBT disposition exist - evidence labels (`static-…`, `diff-cov-…`, `mutation-…`, `pbt-…`) **or** greppable skip/waive lines in `decisions.md` per `docs/mission-artifacts.md`. Static analysis when a project tool runs: **0 warning / 0 error** (else skip/waive). Diff coverage targets touched executable **line and branch ≥90%** (sanity band 90–95%), never global 95–100%. Mutation in scope when any of: `Mutation: required`, pack id `quality`, or `Mutation: high-risk` (`docs/mission-artifacts.md`); then targets **>80%** scoped score (or project higher bar); ordinary missions use `Mutation skipped: not in scope`. PBT: **100%** of design-contract **core-logic** modules need invariants + generators (`fast-check` / Hypothesis / equivalent already in the project) with `pbt-…` evidence, or `Pbt skipped: no project pbt tool` / `Pbt skipped: not core logic` / `Pbt waived: <reason>`. **Must not** invent PBT lib install mid-mission.
