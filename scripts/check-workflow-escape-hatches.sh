#!/bin/sh
# Assert 200-workflow.mdc documents both escape hatches when scope exceeds ≤7:
# (1) same-mission phase split via plan-phaseN.json (or equivalent explicit naming)
# (2) roadmap/multi-mission via spacecraft map
# Vague "Phase 1 / Phase 2" or bare "Large work (>7)" map wording alone is not enough.
set -e
...
echo "ok: 200-workflow.mdc documents phase-split and roadmap/multi-mission escape hatches"
exit 0
